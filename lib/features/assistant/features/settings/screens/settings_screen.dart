...
class _AssistantSettingsScreenState extends ConsumerState<AssistantSettingsScreen> {
  ...
  void didChangeDependencies() {
    ...
    final settings = ref.read(assistantSettingsProvider.notifier).getFor(_assistantId);
    _initial = settings;
  ...
  void _onSave() async {
    ...
    ref.read(assistantSettingsProvider.notifier).save(_assistantId, data);
    setState(() {
      _initial = data;
      _isSaving = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AssistantAppBar(assistantId: _assistantId, subfeatureTitle: 'Settings'),
      body: Form(
        key: _formKey,
        child: ListView(
