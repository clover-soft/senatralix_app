...
class ScriptStep {
  ...
  final String
  spec; // JSON-текст шага (описание команды с применением JSONPath)

  const ScriptStep({required this.id, required this.title, required this.spec});
  ...
  Map<String, dynamic> toJson() => {'id': id, 'title': title, 'spec': spec};
}
...
class Script {
  ...
  factory Script.fromJson(Map<String, dynamic> json) => Script(
    ...
    trigger:
        ((json['trigger'] as String? ?? 'on_dialog_start') == 'on_dialog_end')
        ? ScriptTrigger.onDialogEnd
        : ScriptTrigger.onDialogStart,
    params:
        (json['params'] as Map?)?.map((k, v) => MapEntry('$k', '$v')) ??
        const {},
    steps:
        (json['steps'] as List?)
            ?.map(
              (e) => ScriptStep.fromJson(Map<String, dynamic>.from(e as Map)),
            )
            .toList() ??
        const <ScriptStep>[],
  );
  ...
  Map<String, dynamic> toJson() => {
    ...
    'name': name,
    'enabled': enabled,
    'trigger': trigger == ScriptTrigger.onDialogEnd
        ? 'on_dialog_end'
        : 'on_dialog_start',
    'params': params,
